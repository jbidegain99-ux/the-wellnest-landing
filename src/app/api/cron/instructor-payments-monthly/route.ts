/**
 * Cron mensual de pago a instructores.
 *
 * Schedule: día 1 de cada mes, 12:00 UTC (= 06:00 SV).
 * Calcula el mes calendario anterior completo (día 1 00:00 SV → día 1 del mes
 * siguiente 00:00 SV), genera Excel con desglose por instructor / disciplina /
 * clase y lo envía como adjunto a la lista de destinatarios.
 *
 * Mismo reporte y destinatarios que el cron semanal de los lunes:
 *   env var `INSTRUCTOR_PAYMENTS_RECIPIENT` (coma-separado).
 ...
 *   - Disparo manual con `curl -H "Authorization: Bearer $CRON_SECRET" .../api/cron/instructor-payments-monthly`
 *
...
async function handle(req: NextRequest): Promise<NextResponse> {
  ...
  if (!expected) {
    console.error('[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] CRON_SECRET not configured')
  ...
  console.log(`[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] Computing for ${bounds.label} (${bounds.start.toISOString()} → ${bounds.end.toISOString()})`)

  const result = await computeInstructorPayments(prisma, bounds.start, bounds.end)
  console.log(`[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] ${result.classesCounted} clases, $${result.totalNeto.toFixed(2)} neto, $${result.totalRenta.toFixed(2)} renta`)

  if (result.classesCounted === 0) {
    console.log(`[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] No classes in period, skipping email`)
  ...
  const filename = `pago_instructores_${bounds.label.replace(/\s+/g, '_')}.xlsx`
  ...
  if (!emailResult.success) {
    console.error(`[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] Email failed: ${emailResult.error}`)
    return NextResponse.json({
      ok: false,
      period: bounds.label,
...
// sin alerta significa un mes sin reporte de pagos y nadie se entera.
...
async function handleSafely(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error('[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] Unhandled error:', error)
    const recipient = process.env.ADMIN_NOTIFICATION_EMAIL || 'contact@wellneststudio.net'
    await sendEmail({
      to: recipient,
      subject: '[Wellnest] Falló el cron mensual de pagos a instructores',
      html: `<p>El cron mensual de pagos a instructores falló con el error:</p><pre>${message}</pre><p>Re-disparo manual: GET /api/cron/instructor-payments-monthly?month=YYYY-MM con Authorization Bearer CRON_SECRET.</p>`,
    }).catch((emailErr) => {
      console.error('[CRON_INSTRUCTOR_PAYMENTS_MONTHLY] Alert email also failed:', emailErr)
    })
